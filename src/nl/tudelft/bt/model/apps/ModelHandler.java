...
public abstract class ModelHandler extends ApplicationComponent {
	...
	private int _gridSize; // for solutes concentration
	...
	// biomass parameters
	private float _minimumMassFraction;
	...
	public void initializeSystemSpace() throws InvalidValueException {
		_m.buildSystem(_geometry, _systemSize, _gridSize);
	...
	public void initializeDiffusionReactionSystem() throws ModelException {
		...
		// set the maximum biofilm heigh to leave enough free space
		// to boundary layer
		_m.setMaximumBiofilmSize(_systemSize
	 ...
	 * @param nCells
	 *            number of cells of each species
	 * @param minHeight: minimal x location (height above surface) - a hovering biofilm
	 ...
	 *             if system is not in editable mode
	 */
	protected void inoculateRandomlyAtMinHeightBetweenY1andY2(int[] nCells, float minHeight, float y1, float y2)
			throws NonMatchingNumberException, SystemEditViolationException {
		if (!_physicalSystemIsEditable) {
			throw (new SystemEditViolationException(
					"Tried to innoculate system" + " when not in edit mode"));
		}
		_m.inoculateRandomlyMultispeciesAtMinHeightBetweenY1andY2(nCells, minHeight,y1,y2);
	}
