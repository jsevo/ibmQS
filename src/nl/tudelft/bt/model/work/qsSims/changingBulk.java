...
import nl.tudelft.bt.model.bulkconcentrations.ItermittentBulkConcentration;
 ...
 */
public class changingBulk extends ModelHandler {
	...
	private static float goodThreshold = 6e-3f;
	//private static float goodThreshold = 4e-3f;
	
	private static float inducerThreshold = 0.9f;
	private static float inducerOnsetTime = 15f; // time when inducer is set to bulk concentration

	// Rate of goods production, to fix by ESS analysis
	private static float goodRate1 =0; // [1/T]
	...
	// Inducer
	protected static float inducerBulkConcentration = 1f;
	...
	protected void defineSpeciesAndReactions() throws ModelException {
		...
		inducer.setBulkConcentration(new ItermittentBulkConcentration
				(0f, inducerBulkConcentration, inducerOnsetTime, 1f, 1000f));
		...
		ProcessFactor goodUtil = new StepReturnSubstrateAndGood(substrate, pGood, Ks,
				goodThreshold);
		ProcessFactor step = new Step(inducer, inducerThreshold);
		...
		goodProductionOne.addFactor(step);
		...
		// inducer (I)
		NetReaction rsInducer = new NetReaction(0);
	...
	public static void main(String[] args) {
		...
		ApplicationComponent app = new changingBulk();
