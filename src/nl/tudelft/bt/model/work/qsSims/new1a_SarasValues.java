 ...
 */
public class new1a_SarasValues extends ModelHandler {
	...
	protected static float maxHeight = 1000f; // [um] // only to reproduce Sara's results
	//protected static float simulationFinishTime = 30; // [h]
	
	protected static int totalNumber = 120; // usually 200, here 120 to compare to Sara's
	protected static float ratioOfFirstSpecies = 0.20f;
	...
	public static void main(String[] args) {
		...
		if (args.length < 5) {
			throw new RuntimeException(
					"input arguments missing: \n"
							...
							+ "4: substrate bulk concentration \n"
							+ "5: initial ratio of first species \n"
							+ "6: pGood production rate (1 or 0) \n");
		}
		...
		substrateBulkConcentration = Float.parseFloat(args[3]);
		ratioOfFirstSpecies = Float.parseFloat(args[4]);
		goodRate1 = Float.parseFloat(args[5]);
		...
		ApplicationComponent app = new new1a_SarasValues();
		...
		try {
			...
			Model.model().setMaxBiovolume(2e4f); // only used to reproduce Sara's results
