...
import nl.tudelft.bt.model.reaction.PositiveFeedback;
...
import nl.tudelft.bt.model.reaction.StepOrNoStep;
 ...
 */
public class inducer extends ModelHandler {
	...
	protected static float maxHeight = 100f; // [um]
	//protected static float simulationFinishTime; // [h]
	
	protected static int totalNumber = 200;
	protected static float ratioOfFirstSpecies = 0.5f;
	
	// initial number of particles in the system (inoculum)
	protected static int initialParticleNumber1 = (int) (totalNumber * ratioOfFirstSpecies);
	protected static int initialParticleNumber2 = (int) (totalNumber * (1f - ratioOfFirstSpecies));
	...
	private static float benefit = 6f; // 1.1 //[1/T
	
	// Threshold public good concentration before benefit is received
	private static float goodThreshold = 4e-3f;
	private static float inducerThreshold = -100000f;
	
	private static float inducerEnhancement = 1f;
	private static boolean ifStep = true;

	// Rate of goods production, to fix by ESS analysis
	private static float goodRate1 = 1f; // [1/T]
	
	// Rate of inducer production
	private static float inducerRate1 = 1f; // [1/T]

	private static float costScale = .6f; // 1e-5f; // scales the cost of the
	...
	protected static float substrateBulkConcentration = .1575f;
	private static float substrateDiffusivity = 4e4f; // [um2/h]
	
	// Inducer
	protected static float inducerBulkConcentration = 0f;
	private static float inducerDiffusivity = 4e4f; // [um2/h]
	...
	protected static float systemSize = 163; // [um]
	...
	protected void defineSpeciesAndReactions() throws ModelException {
		...
		inducer.setBulkConcentration(new ConstantBulkConcentration(inducerBulkConcentration));
		...
		//speciesOne.setInducibleColor(pGood, goodThreshold, Color.green, Color.blue);
		...
		//speciesTwo.setInducibleColor(pGood, goodThreshold, Color.yellow, Color.red);

		// 4. Create the Reaction factors, Monod and inhibition coefficients
		// *****************************************************************************
	
		ProcessFactor substrateUtil = new Saturation(substrate, Ks);
		ProcessFactor goodProduction = new Saturation(substrate, Ks);
		ProcessFactor goodUtil = new StepReturnSubstrateAndGood(substrate, pGood, Ks,
				goodThreshold);
		ProcessFactor stepOrNoStep = new StepOrNoStep(inducer, inducerThreshold, ifStep);
		ProcessFactor autoInduction = new PositiveFeedback(inducer, inducerThreshold, 
				inducerEnhancement);
		...
		goodProductionOne = new Reaction("goodProductionOne", activeOne,
				goodRate1, 2);
		goodProductionOne.addFactor(goodProduction);
		goodProductionOne.addFactor(stepOrNoStep);
		...
		inducerProductionOne = new Reaction("inducerProductionOne", activeOne, inducerRate1, 0);
		//inducerProductionOne.addFactor(autoInduction); // positive feedback
	...
	public static void main(String[] args) {
		...
		if (args.length < 3) {
			throw new RuntimeException(
					"input arguments missing: \n"
							...
							+ "4: public good production rate \n"
							+ "5: inclusion of step function or not for pGood production \n");
		}
		...
		goodRate1 = Float.parseFloat(args[3]);
		
		ifStep = (Integer.parseInt(args[4]) == 1);		
			

		// set numerics for multigrid
		MultigridVariable.setSteps(2, 20);
		// create a hande for the application, which will be decorated
		ApplicationComponent app = new inducer();
		...
		try {
			...
			//Model.model().setMaxBiovolume(7e3f);
