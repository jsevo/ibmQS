...
import nl.tudelft.bt.model.apps.components.BulkConcentrationVizualizer;
import nl.tudelft.bt.model.apps.components.SeriesVizualizer;
import nl.tudelft.bt.model.apps.components.VizualModelControler;
...
import nl.tudelft.bt.model.apps.output.TimeConstraintsWriter;
...
/**
 * This program should predict the structural development of an autotrophic biofilm.
 * At first it includes only growth of bacteria on oxygen and ammonium,
 * then as much processes are added until the structural development is alike
 * the one described in Staudt et al. 2004
 * Kinetic parameters are taken from Horn and Hempel 1997 Growth and decay in an
 * auto-/heterotrophic biofilm, Wat. Res. 31(9) 1997.
 * 
 * Because of getting no limitation of substrates yielding a heterogenous structure
 * a yield for Y_X_A was taken from literature (Horn and Hempel 1997). The data
 * for the detached biomass in Amm7 are probably overestimating the detachment rate. 
 * This in turn yields a yield coefficient as applied in AutotrophsEps2 being too high.
 * 
 * @author Roland, Software written by Joao Xavier
 */
public class AutotrophsEps3 extends ModelHandler {
...
//	protected static String outputDirectory = "D:\\0_Delft\\results\\AutoEps";
...
//	the value for the bulk concentration is assumed to be 30g/m^3, actually it should
//	be 45.1g/m^3, but due to numeric problems and experimental data showing until day
//	60 a sufficient supply of ammonium, a constant concentration is assumed
	...
	private static float YEpsA = 0.085f; //[gCOD-A/gCOD-N-NH4]
	...
	protected static float rdetach = 10f; // 
	...
	private static float  endSimulationTime = 1000f;
...
//				ammoniumBulkConcentration));
		ammonium.setBulkConcentration(new ListBulkConcentration(
				"kdfghdih"));
...
//				specificMassH, Color.blue);
	...
	public static void main(String[] args) {
		...
		ApplicationComponent app = new AutotrophsEps3();
...
//		app = new SoluteRateSeriesVizualizer(app);
		...
		try {
			...
			app.addTimedStateWriter(new TimedStateWriterDecorator(new PovRayWriter()));
			app.addTimedStateWriter(new TimedStateWriterDecorator(new SoluteConcentrationWriter()));
			app.addTimedStateWriter(new TimedStateWriterDecorator(new SolidsConcentrationWriter()));
			app.addTimedStateWriter(new TimedStateWriterDecorator(new ParticlePositionWriter()));
...
//			Model.model().setMaximumBiofilmHeight(BiofilmCutOffHeight);
