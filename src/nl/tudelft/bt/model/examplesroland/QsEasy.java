...
/**
 * This program is based on Example1.java, written by Joao Xavier
 * Aim of the simulation is to show the spatial distribution of concentration
 * of a signalling compound that is used for Quorum sensing.
 * At first only an easy first order kinetic is applied for the production of this
 * signalling compound
 * 
 * Note: um representes micro-meter
 * 
 * @author Roland M�hle, Software written by Joao Xavier (j.xavier@tnw.tudelft.nl)
 */
public class QsEasy extends ModelHandler {
	...
	protected static String outputDirectory = "D:\\0_Delft\\results\\EasyQs";
	...
	//SIGNALLING COMPOUND
	//Assumed values
	protected static float QsCompoundBulkConcentration = 0f; //[g/m^3]
	
	private static float QsCompoundDiffusivity = 1.5e-4f; //[m^2/d]
	//
	//Particulate species (biomass X)
	protected static float specificMassX = 341500f; //[g-COD/m^3]

	//Yield coefficients
	private static float YXS = 0.4f; //[gX/gS]

	// Processes
	//Growth (biomass production)
	protected static float uMax = 4.8f; //[d^-1]

	private static float KO2 = 0.35f; //[g/m^3]
	
	//Production rate of signalling compound
	private static float kQs = 0.01f; //[d^-1]
	...
	protected static float relativeBoundaryLayer = 0.01f;
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		//SIGNALLING COMPOUND
		SoluteSpecies QsCompound = new SoluteSpecies("QsCompound",
				QsCompoundDiffusivity);
		//set up the simplest type of bulk concentration: constant
		QsCompound.setBulkConcentration(new ConstantBulkConcentration(
				QsCompoundBulkConcentration));
		...
		growth.addFactor(mO);
		//1st order kinetics for QS production
		Reaction QsProduction = new Reaction("QSproduction", activeX, kQs, 0);
		//
		...
		rsOxygen.addReaction(growth, -(1 / YXS));
		oxygen.setProcesses(rsOxygen);
		// This defines that substrate consumption rate is -(1 / YXS)*rX
		//
		NetReaction rsQsCompound = new NetReaction(1);
		rsQsCompound.addReaction(QsProduction, 1);
		QsCompound.setProcesses(rsQsCompound);
	...
	public static void main(String[] args) {
		...
		ApplicationComponent app = new QsEasy();
