...
public class BiomassParticle implements Cloneable, Serializable {
	...
	private float _radiusChangeInThisIteration;
	...
	public BiomassSpecies.Composition erode(float f) {
		// creiterium changed to 1.1 instead of 1 as value 1 can occure
		// (although chances of this hapening are very low) due to precision
		// of float type
	...
	/**
	 * Move center of bacteria by vector adition with m. Uses setCenter so
	...
	public void move(float x, float y, float z) {
		...
		setCenter(_centerX - x, _centerY - y, _centerZ - z);
	}

	/**
	 * Add contribution of a neighboring to the moving direction if if there is
	 * overlapping.
	 * 
	 * @param neib
	 *            Potentially overlapping neighboring Bacteria
	 * @param m
