...
public class Biofilm3D implements Serializable {
	...
	protected void particlesToFile(FileWriter f) throws IOException {
		ArrayList<BiomassParticle> bl = new ArrayList<BiomassParticle>(Model
				.model().getBiomassAsBiomassParticleCollection());
		Comparator<BiomassParticle> comparator = new BiomassParticle.DepthComparator();
		Collections.sort(bl, comparator);
		for (Iterator<BiomassParticle> iter = bl.iterator(); iter.hasNext();) {
			ParticleWithCapsule s = new ParticleWithCapsule(iter.next());
			f.write(s.toString());
		}
	}
}
